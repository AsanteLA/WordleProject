...
CORRECT_COLOR_BLD = "#0099FF"  # Blue for correct letters
PRESENT_COLOR_BLD = "#FFA500"  # Orange for misplaced letters
MISSING_COLOR_BLD = "#999999"  # Gray for absent letters         # Keys are colored light gray
...
class WordleGWindow:
    ...
    def __init__(self):
        ...
        def key_action(tke):
            ...
            if event.event_type == keyboard.KEY_DOWN:
                keyboard_press = event.name
                #print(event.name)
            if isinstance(tke, str):
                ch = tke.upper()
            else:
                ch = tke.char.upper()
            if ch == "\007" or ch == "\177" or ch == "DELETE" or event.name == 'delete':
                self.show_message("")
                if self._row < N_ROWS and self._col > 0:
                    self._col -= 1
                    sq = self._grid[self._row][self._col]
                    sq.set_letter(" ")
            elif ch == "\r" or ch == "\n" or ch == "ENTER" or event.name == 'enter':
                self.show_message("")
                s = ""
    ...
    def toggle_color_mode(self):
        ...
        self.update_colors()  # Force a color update

    def update_colors(self):
    #Update colors based on the selected color mode."""
        if self._colorblind_mode:
            CORRECT_COLOR = "#0099FF"  # Blue for correct letters
            PRESENT_COLOR = "#FFA500"  # Orange for misplaced letters
            MISSING_COLOR = "#999999"  # Gray for absent letters
        else:
            CORRECT_COLOR = "#55ff00"  # Light green for correct letters
            PRESENT_COLOR = "#ff2d00"  # Brownish yellow for misplaced letters
            MISSING_COLOR = "#999999"  # Gray for letters that don't appear
        for row in range(N_ROWS):
            for col in range(N_COLS):
                self.set_square_color(row, col, self.get_square_color(row, col))  # Reapply colors
        for key in self._keys.values():
            key.set_color(key.get_color())  # Reapply key colors
